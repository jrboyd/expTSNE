...
expTSNE.ui_module= function(id, label = "TSNE"){
  ns = NS(id)
  #ui wrap in ns()
  tags$span(
    shinyjs::useShinyjs(),
...
expTSNE.server_module = function(id){
  moduleServer(
    id,
...
expTSNE.runApp = function(et){
  ...
  server <- function(input, output, session) {
    ...
    server_upload(input, output, session, gene_table, expression_data, custom_gene_sets)
