...
uploadUI = function(Name = "Upload"){
    ...
    sidebarLayout(
        sidebarPanel(
            ...
            fluidRow(
                column(textInput(ns("txt_gene_set_name"), label = "Gene set name", value = "custom"), width = 9),
                column(actionButton(ns("btn_add_gene_set"), label = "Add"), width = 3, style = "margin-top: 24px;",)
            ),
            tags$hr(),
            tabsetPanel(
            ...
            uiOutput(ns("rpt_custom_gene_sets")),
            tags$h5("TODO: gene set import/export"),
            tags$h5("TODO: filtering after upload"),
            tags$h5("TODO: hide example data")
        ),
        mainPanel(
            DT::dataTableOutput(ns("DT_PasteGenes_DataFrame"))
        )
    )
    
}

uploadServer = function(input, output, session, gene_table, tcga_data, custom_gene_sets, Name = "Upload"){
    id = as_id(Name)
    moduleServer(
        id,
