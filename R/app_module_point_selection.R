...
ui_point_selection = function(id = "default_point_selection", ps_size = "400px"){
  ns = NS(id)
  sidebarLayout(
    sidebarPanel(
      tags$h5("TODO: clean up some weird A/B interaction, B trumps A currently."),
      disabled(selectInput(ns("sel_A_clust"), label = "A group clusters", choices = "", multiple = TRUE)),
      disabled(selectInput(ns("sel_B_clust"), label = "B group clusters", choices = "", multiple = TRUE)),
      tags$label("Patient Clustering Methods"),
      tabsetPanel(id = ns("tabs_cluster_method"),
                  tabPanel("knn", #id = "knn", 
                           numericInput(ns("num_nn"), label = "Nearest neighbors", value = 5, min = 2, max = Inf)
                  ), 
                  tabPanel("kmeans", #id = "kmeans", 
                           numericInput(ns("num_kmeans"), label = "k", value = 5, min = 2, max = Inf)
                  ), 
                  tabPanel("hierarchical", #id = "hierarchical", 
                           numericInput(ns("num_clust"), label = "n_clust", value = 5, min = 2, max = Inf)
                  )
      ),
      withSpinner(plotOutput(ns("plot_tsne_clusters"), width = ps_size, height = ps_size))
    ),
    mainPanel(
      withSpinner(plotOutput(ns("plot_A_group"), width = ps_size, height = ps_size,
...
server_point_selection = function(id = "default_point_selection", tsne_clust, meta_data, tsne_res){
  moduleServer(
    id,
